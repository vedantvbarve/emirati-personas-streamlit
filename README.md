...
- **Automatic Persona Extraction:** Bot name and origin are extracted from each `[persona files].txt` file in the `Personas` folder, ensuring responses are tailored. The assumption that the line with name and origin begin with `Name: ` and `Origin: ` respectively
