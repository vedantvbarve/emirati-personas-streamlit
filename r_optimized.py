...
# Load traits and languages if not already loaded
if not st.session_state.available_traits:
...
# Setup Phase - Trait and Language Selection
if not st.session_state.setup_complete:
    ...
    st.markdown("Choose the traits you want the AI persona to focus on:")
    ...
    if st.session_state.available_traits:
        ...
        for i, trait in enumerate(st.session_state.available_traits):
            with cols[i % 3]:
                if st.checkbox(trait, key=f"trait_{i}"):
                    selected_traits.append(trait)
        st.session_state.selected_traits = selected_traits
    else:
        st.error("No traits found. Please ensure traits.txt exists in the TO_INPUT folder.")
    ...
    if st.session_state.available_languages:
        ...
        st.error("No languages found. Please ensure languages.txt exists in the TO_INPUT folder.")
    ...
    with col2:
        if st.button("✅ Done", type="primary", use_container_width=True):
            if st.session_state.selected_traits and st.session_state.selected_language:
                ...
                st.error("Please select at least one trait and a language.")
    ...
    if st.session_state.selected_traits or st.session_state.selected_language:
        ...
        if st.session_state.selected_traits:
            st.write(f"**Selected Traits:** {', '.join(st.session_state.selected_traits)}")
