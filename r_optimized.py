...
"""r_optimized.ipynb Automatically generated by Colab. Original file is located at https://colab.research.google.com/drive/15bLkrgQW-nSRvDS5x_Xl04kduyMlfT8O """
...
def load_traits():
    """Load traits from traits.txt file"""
    try:
...
def load_languages():
    """Load languages from languages.txt file"""
    try:
...
# Initialize with setup_complete as False to force setup
defaults = {
    'response_matrix': [],
    ...
    'setup_complete': False,  # This ensures setup appears first
    'available_traits': [],
    'available_languages': []
}
...
# Force load traits and languages every time
...
# SETUP PHASE - ALWAYS APPEARS FIRST
if not st.session_state.setup_complete:
    st.title("🇱🇰 Sri Lankan Persona Chat Setup")
    st.markdown("### Please configure your chat preferences before starting")
    st.markdown("---")
    ...
    if st.session_state.available_traits:
        # Use multiselect instead of checkboxes for better UX
        selected_traits = st.multiselect(
            "Select traits:",
            options=st.session_state.available_traits,
            default=[],
            help="You can select multiple traits"
        )
        ...
        st.error("❌ No traits found. Please ensure traits.txt exists in the same folder as the script.")
    ...
    if st.session_state.available_languages:
        ...
        st.error("❌ No languages found. Please ensure languages.txt exists in the same folder as the script.")
    ...
    with col2:
        if st.button("✅ Done - Start Chatting!", type="primary", use_container_width=True):
            if st.session_state.selected_traits and st.session_state.selected_language:
                st.session_state.setup_complete = True
                st.success("🎉 Setup completed! Redirecting to chat...")
                time.sleep(1)
                st.rerun()
            else:
                st.error("❌ Please select at least one trait and a language before proceeding.")
    ...
    if st.session_state.selected_traits or st.session_state.selected_language:
        ...
        if st.session_state.selected_traits:
            ...
            st.warning("**No traits selected yet**")
            
        if st.session_state.selected_language:
            st.info(f"**Selected Language:** {st.session_state.selected_language}")

# MAIN APP - Only show after setup is complete
else:
    # Reset setup button at the top
    col1, col2 = st.columns([6, 1])
    with col2:
        if st.button("🔄 Change Setup", key="reset_setup"):
