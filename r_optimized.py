...
if "traits_setup_done" not in st.session_state:
    st.session_state.traits_setup_done = False
...
defaults = {
    ...
    'user_input': "",
    'conversation_events': [],
    'selected_traits': st.session_state.available_traits,  # Default: all traits
    'selected_language': "English"  # Default: English
}
...
if persona_files:
    ...
    if st.session_state.selected_persona and st.session_state.questions:
        ...
        # TRAITS AND LANGUAGE SELECTION SECTION
        if not st.session_state.traits_setup_done:
            st.markdown("---")
            st.subheader("🎯 Customize Your Chat Experience")
            
            # Traits Selection
            st.markdown("**Select Personality Traits** (Choose one or more):")
            selected_traits = st.multiselect(
                "Select traits:",
                options=st.session_state.available_traits,
                default=st.session_state.selected_traits,
                help="Choose which personality traits you want the AI to focus on"
            )
            
            # Language Selection
            st.markdown("**Select Language** (Choose one):")
            selected_language = st.selectbox(
                ...
                help="Choose the language for conversations"
            )
            ...
            if selected_traits:
                st.write(f"• **Traits:** {', '.join(selected_traits)}")
            else:
                st.write(f"• **Traits:** All traits (default)")
            st.write(f"• **Language:** {selected_language}")
            
            st.markdown("---")
            st.info("👆 Please complete the setup above to start chatting!")
            ...
            with col2:
                if st.button("🔄 Change", key="reset_setup"):
                    st.session_state.traits_setup_done = False
                    st.rerun()
