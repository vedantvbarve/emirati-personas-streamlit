...
# ===== FORCE SETUP TO APPEAR FIRST =====
# Check if we have the required setup parameters
setup_required = True
...
# Check if we have valid selections to skip setup
if "selected_traits" in st.session_state and "selected_language" in st.session_state:
...
defaults = {
    ...
    'user_input': "",
    'conversation_events': [],
    'selected_traits': [],  # Empty until user selects
...
# ==========================================
# SETUP PHASE - APPEARS WHEN SETUP IS REQUIRED
# ==========================================
if setup_required:
    ...
    # Force show the debug info
    st.info(f"Setup Required: {setup_required}")
    st.info(f"Available Traits: {len(st.session_state.available_traits)} traits")
    st.info(f"Available Languages: {len(st.session_state.available_languages)} languages")
    ...
    if st.session_state.available_traits:
        ...
        for i, trait in enumerate(st.session_state.available_traits):
            with cols[i % 3]:
                if st.checkbox(trait, key=f"setup_trait_{i}"):
...
with col2:
    if st.button("🔄 Reset Setup", key="reset_setup"):
        # Clear selections to force setup
